...
const negHeadline = [
  ...
  `And we lost the war. I knew you weren't ready.`,
  `No wonder machines do this now.`,
  `Don't you know how numbers work?`,
  `That was the worst square you could have picked.`,
  `Horrible choice. Just horrible.`,
  `If you want to help us, you can join our enemy.`,
];

const playerMusic = new Audio(
  "audio/Gustav_Holst_-_the_planets,_op._32_-_i._mars,_the_bringer_of_war-[AudioTrimmer.com].mp3"
);
...
function checkCorners(id, num) {
  ...
  if (
    ...
    squareArray[id + long].flagged === true &&
    squareArray[id + lat].mine === true
  ) {
    return false;
  } // Both paths contain Mines
...
function clearSquare(id) {
  ...
  squareEl.style.backgroundImage =
    "radial-gradient(circle, #ffffff, #fcfafc, #faf5f7, #f9f0ef, #f5ece7, #f4e8de, #f0e5d4, #e9e2cb, #e7debd, #e6d9af, #e4d5a2, #e2d094)";
...
function bigWinner() {
  ...
  if (tutorial === true) {
    ...
    dispMessageEl.innerText = `Great job! This next one has ${
      startMines + 5
    } mines!`;
...
function checkWin() {
  ...
  for (const i in squareArray) {
    ...
    if (
      (squareArray[i].cleared === true && squareArray[i].mine === false) ||
      // if mines are marked
      (squareArray[i].mine === true && squareArray[i].flagged === true)
    ) {
      winCount += 1;
    }
  }
...
function headline(pos) {
  // Positive headline
  if (pos === true) {
    dispMessageEl.innerText =
      posHeadline[Math.floor(Math.random() * posHeadline.length)];
  } // Negative headline
  else {
    dispMessageEl.innerText =
      negHeadline[Math.floor(Math.random() * negHeadline.length)];
  }
}
...
function flagSquare(obj, id) {
  ...
  if (squareArray[id].flagged === true) {
    ...
    obj.style.backgroundImage =
      "radial-gradient(circle, #5c4a0a, #65510d, #6d5710, #765e13, #7f6516, #81681b, #846a20, #866d25, #826c2c, #7e6b33, #7b6939, #77683f)";
    mineCount();
    checkWin();
    ...
    obj.style.backgroundImage =
      "radial-gradient(circle, #8f0000, #a04518, #b06e3b, #bf9465, #cfb995, #d7c7a7, #e0d4b9, #e9e2cb, #e7debd, #e6d9af, #e4d5a2, #e2d094)";
    mineCount();
    checkWin();
  }
}
...
function playAgain() {
  ...
  toggleEl.style.backgroundImage =
    "radial-gradient(circle, #ffffff, #fffcfe, #fff9fa, #fff6f4, #fff5ec, #f4ebd8, #e4e2c6, #cedab8, #adc799, #87b47f, #5da268, #1e9056)";
...
function reset(e) {
  ...
  toggleEl.style.backgroundImage =
    "radial-gradient(circle, #ebe8dd, #ebe8dd, #ebe8dd, #ebe8dd, #ebe8dd, #eae6d7, #eae4d1, #e9e2cb, #e7debd, #e6d9af, #e4d5a2, #e2d094)";
...
function revealBoard(id) {
  ...
  for (let i = 0; i < squareArray.length; i++) {
    if (squareArray[i].cleared === false && squareArray[i].flagged === false) {
      ...
      blockEl[i].style.backgroundImage =
        "radial-gradient(circle, #bb7617, #bb7617, #bb7617, #bb7617, #bb7617, #b67316, #b16f16, #ac6c15, #a06514, #955e13, #8a5711, #7f5010)";
    }
    // Successfully marked mines
    if (squareArray[i].mine === true && squareArray[i].flagged === true) {
      blockEl[i].style.backgroundImage =
        "radial-gradient(circle, #4d3434, #6c565a, #8a7a80, #aaa0a7, #ccc8cd, #d2cdd3, #d9d2d8, #e0d7de, #ceb9bf, #bb9d9b, #a38475, #826f52)";
    } else if (squareArray[i].mine === true) {
      // Not marked mines
      blockEl[i].style.backgroundImage =
        "radial-gradient(circle, #d16b6b, #c36265, #b45a5e, #a65158, #984951, #99484c, #9a4747, #9b4742, #a8503b, #b25b32, #b86726, #bb7617)";
    } else if (
...
function assignSurrounding() {
  ...
  for (let id = 0; id < squareArray.length; id++) {
    ...
    // Top Left
      ...
      if (squareArray[1].mine === true) squareArray[0].surroundingMines += 1;
      if (squareArray[10].mine === true) squareArray[0].surroundingMines += 1;
      if (squareArray[11].mine === true) squareArray[0].surroundingMines += 1;
    }
    // Top Right
    else if (id === 9) {
      if (squareArray[8].mine === true) squareArray[9].surroundingMines += 1;
      if (squareArray[19].mine === true) squareArray[9].surroundingMines += 1;
      if (squareArray[18].mine === true) squareArray[9].surroundingMines += 1;
    }
    // Bottom Left
    else if (id === 90) {
      if (squareArray[80].mine === true) squareArray[90].surroundingMines += 1;
      if (squareArray[81].mine === true) squareArray[90].surroundingMines += 1;
      if (squareArray[91].mine === true) squareArray[90].surroundingMines += 1;
    }
    // Bottom Right
    else if (id === 99) {
      if (squareArray[98].mine === true) squareArray[99].surroundingMines += 1;
      if (squareArray[88].mine === true) squareArray[99].surroundingMines += 1;
      if (squareArray[89].mine === true) squareArray[99].surroundingMines += 1;
    }
    // Middle of grid
    else {
      if (squareArray[id - 11].mine === true)
...
function resetSelect() {
  ...
  //
    ...
    timerEl.innerText = "01 : 00";
    dispMessageEl.innerText =
      "Choose the game mode below, then click on the grid to begin!";
    resetLikeVariables();
  }
  //
...
function toggleButton(e) {
  ...
  if (toggleEl.innerText === "START") {
    ...
    timerEl.style.backgroundImage =
      "radial-gradient(circle, #f2d1d7, #f2d1d7, #f2d1d7, #f2d1d7, #f2d1d7, #f5c7d0, #f8bdc9, #fab3c3, #fd9cb5, #ff83a9, #ff679d, #ff4593)";
    dispMessageEl.innerText =
      "Welcome to MEGA Minesweeper! Do you need a tutorial?";
...
function noTutorial() {
  resetButtons();
  dispMessageEl.innerText =
    "Choose the game mode below, then click anywhere on the grid to start!";
}
...
function yesTutorial() {
  ...
  tutorial = true;
  dispMessageEl.innerText =
    "The grid below represents a minefield. Go ahead and click anywhere on the grid to start.";
}
//----------------TUTORIAL--------------------------------
function casualTeachOne() {
  dispMessageEl.innerText =
    "The first square will never be a mine. In CASUAL mode, the clock ticks up, so no rush!";
  setTimeout(casualTeachTwo, 5000);
}

function casualTeachTwo() {
  dispMessageEl.innerText =
    "You can click on the CLEAR button to toggle between CLEAR and FLAG.";
  toggleEl.addEventListener("click", toggleButton);
}

function casualTeachThree() {
  dispMessageEl.innerText =
    "Each number represents how many mines surround that square.";
  setTimeout(casualTeachFour, 5000);
}

function casualTeachFour() {
  dispMessageEl.innerText =
    "Try to CLEAR the squares without mines, and FLAG the squares with mines!";
  playerMusic.play();
}
